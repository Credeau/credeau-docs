...
--data '{
    ...
    "aadhar_number_first_2_digits_mandatory": "56",
...
|--------------------|--------|----------|-------------|
| `user_id`          | string | Yes      | Unique identifier of the user |
| `bank_name`        | string | No       | Name of user received from Bank
| `pan_name`         | string | No       | Name of user received from PAN
| `bank_dob`         | string | No       | DOB of user received from Bank in `YYYY-MM-DD` format
| `pan_dob`          | string | No       | DOB of user received from PAN in `YYYY-MM-DD` format
| `bank_father_name` | string | No       | Name of father received from Bank
| `pan_father_name`  | string | No       | Name of father received from PAN

#### Request cURL

```bash
curl --location 'https://fraudgator.credeau.com/api/bank-pan-binding' \
--header 'x-client-id: <client_id>' \
--header 'x-auth-token: <auth_token>' \
--header 'Content-Type: application/json' \
--data '{
...
1. When all the fields match -
    ...
    ```json
    {
        ...
        "bank_pan_dob_match": true,
        "bank_pan_father_name_match": true,
        "overall_match": true
    }
    ```
...
3. When none of the fields match -
    ...
    ```json
    {
        ...
        "user_id": "test_user_123",
        "bank_pan_name_match": false,
        "bank_pan_dob_match": false,
        "bank_pan_father_name_match": false,
        "overall_match": false
    }
    ```
